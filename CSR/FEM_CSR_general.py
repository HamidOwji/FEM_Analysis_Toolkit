...
def assemble_global_stiffness(elements, E, nu, t):
    ...
    for elem in elements:
        k = compute_stiffness_matrix_for_rectangular_element(elem['coords'], E, nu, t)
        for i in range(4):
            for j in range(4):
...
def compute_element_forces(element, U, D):
    ...
    k = compute_stiffness_matrix_for_rectangular_element(element['coords'], D)
...
def compute_element_stress(element, U, D, t):
    ...
    for gp, weight in zip(gauss_points, weights):
        B = compute_B_matrix_for_rectangular_element(element['coords'], gp)
...
K_global = assemble_global_stiffness(elements, E, nu, t)
