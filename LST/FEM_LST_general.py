import numpy as np
from Mesh_Tri6_extractor import generate_elements_for_stiffness, generate_elements_for_plot, read_mesh_data
import FEM_LST_plotting  # Import the new FEM_LST_plotting module
from Stiffness_LST import compute_stiffness_matrix_for_6node_element, compute_B_matrix_for_6node_element
...
def assemble_global_stiffness(elements, E, nu, t):
    ...
    for elem in elements:
        k = compute_stiffness_matrix_for_6node_element(elem['coords'], E, nu, t)
        for i in range(6):
            for j in range(6):
...
def compute_element_forces(element, U, D):
    ...
    k = compute_stiffness_matrix_for_6node_element(element['coords'], D)
...
def compute_element_stress(element, U, D, t):
    ...
    for gp, weight in zip(gauss_points, weights):
        B = compute_B_matrix_for_6node_element(element['coords'], gp)
...
# Example: Apply a load of 1000 in the x direction to node 3
...
# for elem in elements_for_stiffness:
...
# FEM_LST_plotting.plot_mesh(elements_for_plot, node_coordinates)
# FEM_LST_plotting.plot_displacements(node_coordinates, U, 'stress')
# FEM_LST_plotting.plot_mesh_with_boundary_conditions(elements_for_plot, node_coordinates, left_boundary_nodes)
## FEM_LST_plotting.plot_loads(node_coordinates, F_external, 1)
FEM_LST_plotting.plot_mesh_with_loads(elements_for_plot, node_coordinates, left_boundary_nodes, F_external)
# FEM_LST_plotting.plot_displacements(node_coordinates, U_full, 'Nodal Displacements', scale_factor=10000)
